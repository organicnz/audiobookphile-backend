...
Deno.serve(async (req) => {
  ...
  if (typeof cronSecret === 'string' && cronSecret.length > 0 && authHeader === `Bearer ${cronSecret}`) {
    isAdmin = true
  } else if (authHeader) {
    ...
    if (user) {
      const { data: profile } = await adminClient.from('profiles').select('user_type').eq('id', user.id).single()
  ...
  try {
    if (req.method === 'POST') {
      ...
      const { count } = await adminClient.from('library_items').select('*', { count: 'exact', head: true }).or('cover_path.is.null,cover_path.like./%')
      ...
      const processCoversAsync = async () => {
        ...
        for (const item of items) {
          ...
          if (title) {
            console.log(`[Sync Covers] Fetching cover for: "${title}" by ${author}...`)
            try {
              ...
              if (fetchRes && fetchRes.cover && fetchRes.cover.buffer) {
                ...
                console.log(`[Sync Covers] Uploading to covers/${storagePath}...`)

                const { error: uploadError } = await adminClient.storage.from('covers').upload(storagePath, fileData.buffer, {
                  upsert: true,
                  contentType
                })
              ...
              console.error(`[Sync Covers] Error processing ${title}:`, e.message)
              errorCount++
            }
          }
        }
        console.log(`[Sync Covers] Background task complete. Success: ${successCount}, Not Found: ${notFoundCount}, Errors: ${errorCount}`)
      }
