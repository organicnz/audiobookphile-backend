import { GetObjectCommand, HeadObjectCommand, S3Client } from 'npm:@aws-sdk/client-s3@^3.693.0'
...
export class StorageRouter {
  ...
  async getSignedUrl(path: string, expiresIn: number): Promise<string> {
    if (path.startsWith('supabase://')) {
      ...
      const { data, error } = await this.supabase.storage.from('audio-files').createSignedUrl(actualPath, expiresIn)
  ...
  async fileExists(path: string): Promise<boolean> {
    if (path.startsWith('supabase://')) {
      ...
      const { data } = await this.supabase.storage.from('audio-files').list(folder, { search: filename })
