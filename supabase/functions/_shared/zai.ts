 ...
 */
export async function sortFilesWithZAI(filenames: string[], zaiApiKey: string): Promise<string[]> {
  ...
  if (cached) {
    console.log(`[z.ai] Cache hit for ${filenames.length} track sequence sorting.`)
    return cached
  }

  try {
    const prompt = `You are an expert audiobook librarian. Sort these audiobook chapter/track filenames into exact narrative chronological reading order.
Consider:
...
Return ONLY a valid JSON array of strings containing every exact filename in chronological order: ["file1.mp3", "file2.mp3", ...]`
    ...
    if (res.ok) {
      ...
      if (
        ...
        console.log(`[z.ai] Successfully optimized chapter sequence sorting for ${filenames.length} tracks.`)
...
export async function matchExistingBookWithZAI(
  ...
  if (!uploadTitle || !existingBooks || existingBooks.length === 0 || !zaiApiKey) {
    return null
  }

  const cacheKey = `match_book_${uploadTitle.toLowerCase().trim()}||${uploadAuthor.toLowerCase().trim()}||${existingBooks.map((b) => b.id).join(',')}`
  ...
  try {
    ...
    const prompt = `An audiobook is being uploaded with Title: "${uploadTitle}" and Author: "${uploadAuthor || 'Unknown'}".
...
If it is a completely new/different book, return: {"matchedId": null}`
    ...
    if (res.ok) {
      ...
      if (match) {
        ...
        if (result.matchedId && existingBooks.some((b) => b.id === result.matchedId)) {
          console.log(`[z.ai] Matched uploading book "${uploadTitle}" to existing record ID: ${result.matchedId}`)
...
export async function smartSortLibraryItems(
  ...
  const defaultSortedIds = [...items].sort((a, b) => (a.title || '').localeCompare(b.title || '', undefined, { numeric: true })).map((i) => i.id)

  if (items.length <= 1 || !zaiApiKey) return defaultSortedIds

  const cacheKey = `smart_sort_${criteria}||${items.map((i) => i.id).join(',')}`
...
Return ONLY a valid JSON array of string IDs representing the sorted order: ["id1", "id2", ...]`
    ...
    if (res.ok) {
      ...
      if (sortedIds && sortedIds.length > 0) {
        const missingIds = defaultSortedIds.filter((id) => !sortedIds.includes(id))
        const validSortedIds = sortedIds.filter((id) => defaultSortedIds.includes(id))
...
export async function enrichMetadataWithZAI(
  ...
  author: string,
  zaiApiKey: string
): Promise<{ description?: string; genres?: string[]; publishedYear?: string } | null> {
  if (!title || !zaiApiKey) return null

  const cacheKey = `enrich_meta_${title.toLowerCase().trim()}||${(author || '').toLowerCase().trim()}`
  const cachedMeta = getCachedResult<{ description?: string; genres?: string[]; publishedYear?: string }>(cacheKey)
    ...
    const res = await fetch('https://open.bigmodel.cn/api/paas/v4/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: `Provide accurate executive summary (description), top 3 genres/tags, and published year for the audiobook "${title}" by "${
              author || 'Unknown'
...
export async function generateChapterAIInsights(
  ...
  const cacheKey = `chapter_insights_${title.toLowerCase().trim()}||${chapterIndex ?? 0}||${chapterTitle.toLowerCase().trim()}`
  const cachedInsights = getCachedResult<{ summary: string; keyTakeaways: string[]; mood: string }>(cacheKey)
  if (cachedInsights) return cachedInsights

  try {
    const prompt = `You are an expert literary scholar and audiobook companion. 
Provide a concise, high-level executive summary and key takeaways for Chapter ${chapterIndex ?? ''}: "${chapterTitle}" from the audiobook "${title}" by ${
