 ...
 */
export async function fetchAuthorAvatar(adminClient: SupabaseClient, author: { id: string; name: string }): Promise<string | null> {
  ...
  try {
    const wikiRes = await fetch(
      `https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(author.name)}&prop=pageimages&format=json&pithumbsize=500&redirects=1`
    );
    if (wikiRes.ok) {
    ...
    console.log(`[Avatar Fetcher] Wikipedia fetch failed for ${author.name}`, e.message);
  }
  ...
  if (!storagePath) {
    try {
      const res = await fetch(
        `https://openlibrary.org/search/authors.json?q="${encodeURIComponent(author.name)}"&limit=1`,
      );
      ...
      if (res.ok) {
        ...
        if (doc?.key) {
          const keyPath = doc.key.startsWith("/authors/") ? doc.key : `/authors/${doc.key}`;
      ...
      console.log(`[Avatar Fetcher] OpenLibrary fetch failed for ${author.name}`, e.message);
    }
  }
  ...
  if (!storagePath) {
    try {
      ...
      const diceBearUrl = `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(author.name)}&backgroundColor=000000,1a1a1a,333333&textColor=ffffff&fontWeight=600`;
      ...
      console.log(`[Avatar Fetcher] DiceBear fetch failed for ${author.name}`, e.message);
    }
  }
