...
export async function fetchAuthorAvatar(adminClient: SupabaseClient, author: { id: string; name: string }): Promise<string | null> {
        ...
        if (pageId !== '-1' && pages[pageId]?.thumbnail?.source) {
          ...
          if (imgRes.ok) {
            ...
            if (buf.byteLength > 5000) {
              ...
              const { error: uploadErr } = await adminClient.storage.from('covers').upload(storagePath, buf, {
                upsert: true,
  ...
  if (!storagePath) {
    try {
      const res = await fetch(`https://openlibrary.org/search/authors.json?q="${encodeURIComponent(author.name)}"&limit=1`)
