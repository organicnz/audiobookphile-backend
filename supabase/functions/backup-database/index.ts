...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isCron = typeof cronSecret === 'string' && cronSecret.length > 0 && authHeader === `Bearer ${cronSecret}`
    ...
    // 2. Upload to storage
    const { error: uploadErr } = await supabase.storage.from('backups').upload(filename, backupJson, {
