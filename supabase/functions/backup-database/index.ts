...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verify user is an admin or the request comes from a cron job with CRON_SECRET or SERVICE_ROLE_KEY
    ...
    // 2. Upload to storage
    const { error: uploadErr } = await supabase.storage.from("backups").upload(
      filename,
      backupJson,
      {
