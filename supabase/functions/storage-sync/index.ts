...
Deno.serve(async (req) => {
  ...
  const listSupabaseAudio = async () => {
    const { data, error } = await adminClient.storage.from('audio-files').list()
    if (error) return []
    return (data || [])
  ...
  const listB2Objects = async () => {
    ...
    try {
      ...
      const data = await s3.send(new ListObjectsV2Command({ Bucket: Deno.env.get('B2_BUCKET_NAME') }))
  ...
  const buildReport = async () => {
    ...
    // get DB paths
    const { data: books } = await adminClient.from('library_items').select('cover_path, audio_files')
