...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const activeTier = Deno.env.get('ACTIVE_B2_TIER') === 'secondary' ? 'secondary' : 'primary'

    if (activeTier === 'secondary' && Deno.env.get('B2_SECONDARY_ENDPOINT') && Deno.env.get('B2_SECONDARY_BUCKET_NAME')) {
      ...
      return new Response(JSON.stringify({ url, provider_prefix: 'b2-secondary://' }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
      ...
      const { data, error } = await supabase.storage.from('audio-files').createSignedUploadUrl(filename, { upsert: true })

      if (error || !data?.signedUrl) {
        throw new Error(`Supabase presign error: ${error?.message}`)
      }

      return new Response(JSON.stringify({ url: data.signedUrl, provider_prefix: 'supabase://' }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
