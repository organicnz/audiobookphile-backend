...
serve(async (req) => {
  ...
  try {
    ...
    if (!filename) {
      return new Response(JSON.stringify({ error: 'Filename is required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (Deno.env.get('B2_ENDPOINT') && Deno.env.get('B2_BUCKET_NAME')) {
      ...
      return new Response(JSON.stringify({ url: signedUrl, provider_prefix: 'supabase://' }), { 
        status: 200, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      })
    }
