...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: item, error: itemError } = await supabase.from('library_items').select('*, books(audio_files, chapters)').eq('id', itemId).single()
      ...
      await Promise.all(
        audioFiles.map(async (audioFile, idx) => {
          ...
          try {
            ...
            console.warn(`[playback-start] Missing storage file at "${storagePath}": ${signErr.message}`)
            return null
          }
        })
      )
    ...
    let progressQuery = supabase.from('media_progress').select('current_time_pos').eq('user_id', user.id).eq('library_item_id', itemId)
