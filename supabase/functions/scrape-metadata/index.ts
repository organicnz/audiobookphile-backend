...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!title) {
      return new Response(JSON.stringify({ error: 'Item has no title to search' }), { status: 400, headers: corsHeaders })
    }
    ...
    const scrapeAndPatch = async () => {
      ...
      try {
        ...
        if (data.items && data.items.length > 0) {
          ...
          if ((!book.genres || (Array.isArray(book.genres) && book.genres.length === 0)) && volumeInfo.categories) {
          ...
          if (updateNeeded) {
            console.log(`Found missing metadata, patching books id: ${item.media_id}`)
            const { error: updateErr } = await supabase.from('library_items').update(updates).eq('id', item.media_id)
        ...
        if ((!book.description || !book.genres || book.genres.length === 0) && zaiApiKey) {
          console.log(`Enriching metadata with Z.ai GLM-4 for: ${title}`)
          const enriched = await enrichMetadataWithZAI(title, authorName, zaiApiKey)
          if (enriched) {
            const aiUpdates: any = {}
            if (!book.description && enriched.description) {
              aiUpdates.description = enriched.description
            }
            if ((!book.genres || book.genres.length === 0) && enriched.genres) {
              aiUpdates.genres = enriched.genres
            }
            if (!book.published_year && enriched.publishedYear) {
              aiUpdates.published_year = String(enriched.publishedYear).substring(0, 4)
            }

            if (Object.keys(aiUpdates).length > 0) {
              await supabase.from('library_items').update(aiUpdates).eq('id', item.media_id)
              console.log(`Successfully enriched metadata via Z.AI for book: ${title}`)
            }
          }
        }
      } catch (e) {
