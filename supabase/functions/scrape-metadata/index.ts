...
serve(async (req) => {
  ...
  try {
    ...
    if (fetchErr || !item) {
      return new Response(JSON.stringify({ error: 'Library item not found' }), { status: 404, headers: corsHeaders })
    }

    if (item.media_type !== 'book' || !item.media_id) {
      return new Response(JSON.stringify({ error: 'Only books are supported for metadata scraping' }), { status: 400, headers: corsHeaders })
    }
    ...
    if (bookErr || !book) {
      return new Response(JSON.stringify({ error: 'Book record not found' }), { status: 404, headers: corsHeaders })
    }
    ...
    return new Response(JSON.stringify({ success: true, message: 'Scraping started asynchronously' }), { 
      status: 202, 
