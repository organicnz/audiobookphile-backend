...
Deno.serve(async (req) => {
  ...
  if (
    (envCronSecret && authHeader === `Bearer ${envCronSecret}`) ||
  ...
  try {
    if (req.method === "POST") {
      ...
      const { data: authors, error: authorsError } = await adminClient.from(
        "authors",
      ).select("id, name").is("image_path", null).limit(limit);

      if (authorsError) throw authorsError;

      const { count } = await adminClient.from("authors").select("*", {
        count: "exact",
        head: true,
      }).is("image_path", null);
      ...
      const matchUnlinkedAuthorsAsync = async () => {
        ...
        const unlinkedItems = items.filter((item: any) =>
          !item.book_authors || item.book_authors.length === 0
        );
