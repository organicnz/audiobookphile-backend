...
Deno.serve(async (req) => {
  ...
  try {
    if (req.method === 'POST') {
      ...
      const { data: authors, error: authorsError } = await adminClient.from('authors').select('id, name').is('image_path', null).limit(limit)

      if (authorsError) throw authorsError

      const { count } = await adminClient.from('authors').select('*', { count: 'exact', head: true }).is('image_path', null)

      const matchUnlinkedAuthorsAsync = async () => {
        const { data: items } = await adminClient
          .from('library_items')
          .select('id, library_id, author_names_first_last, book_authors(author_id)')
          .not('author_names_first_last', 'is', null)

        if (!items) return
        const unlinkedItems = items.filter((item: any) => !item.book_authors || item.book_authors.length === 0)

        for (const item of unlinkedItems) {
          const rawAuthorStr = item.author_names_first_last?.trim()
          if (!rawAuthorStr || !item.library_id) continue

          const rawAuthors = rawAuthorStr
            .split(/\s*(?:\/|,|&|\band\b)\s*/i)
            .map((a: string) => a.trim())
            .filter(Boolean)
      ...
      const processAuthorsAsync = async () => {
        try {
          ...
          console.error('[Sync Authors] Unlinked author matching error:', e.message)
        }
