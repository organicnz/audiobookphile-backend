...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { count: totalUsers, error: usersErr } = await supabase.from('profiles').select('*', { count: 'exact', head: true })

    if (usersErr) throw new Error(usersErr.message)

    // 2. Get total libraries count
    const { count: totalLibraries, error: libsErr } = await supabase.from('libraries').select('*', { count: 'exact', head: true })

    if (libsErr) throw new Error(libsErr.message)

    // 3. Get total library items count
    const { count: totalItems, error: itemsErr } = await supabase.from('library_items').select('*', { count: 'exact', head: true })
    ...
    try {
      ...
      const { count, error: sessionsErr } = await supabase.from('media_progress').select('*', { count: 'exact', head: true }).gte('last_update', twoHoursAgo)
