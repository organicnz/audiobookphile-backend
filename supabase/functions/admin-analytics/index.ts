...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { count: totalUsers, error: usersErr } = await supabase.from(
      "profiles",
    ...
    const { count: totalLibraries, error: libsErr } = await supabase.from(
      "libraries",
    ).select("*", { count: "exact", head: true });
    ...
    const { count: totalItems, error: itemsErr } = await supabase.from(
      "library_items",
    ).select("*", { count: "exact", head: true });
    ...
    try {
      const twoHoursAgo = new Date(Date.now() - 2 * 60 * 60 * 1000)
        .toISOString();
      const { count, error: sessionsErr } = await supabase.from(
        "media_progress",
      ).select("*", { count: "exact", head: true }).gte(
        "last_update",
        twoHoursAgo,
      );
