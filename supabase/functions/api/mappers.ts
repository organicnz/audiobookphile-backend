...
export function mapBookForMobile(
    ...
    .map(({ af, meta, size }) => {
      ...
      return {
        ...
        metadata: {
          ...
          relPath: rel_path,
          size: size,
          mtimeMs: Number(meta.mtimeMs || meta.mtime_ms) ||
            new Date().getTime(),
          ctimeMs: Number(meta.ctimeMs || meta.ctime_ms) ||
            new Date().getTime(),
  ...
  const userMediaProgress = progressRecord
    ? {
      ...
      startedAt: progressRecord.started_at
        ? new Date(progressRecord.started_at).getTime()
        : null,
      finishedAt: progressRecord.is_finished &&
          (progressRecord.finished_at || progressRecord.last_update)
        ? new Date(progressRecord.finished_at || progressRecord.last_update!)
          .getTime()
        : null,
    }
    : null;
  ...
  return {
    ...
    mtimeMs: String(item.updated_at) || String(item.mtime)
      ? new Date(String(item.updated_at) || String(item.mtime)).getTime()
      : Date.now(),
    ctimeMs: String(item.created_at) || String(item.ctime)
      ? new Date(String(item.created_at) || String(item.ctime)).getTime()
      : Date.now(),
    birthtimeMs: String(item.created_at) || String(item.birthtime)
      ? new Date(String(item.created_at) || String(item.birthtime)).getTime()
      : Date.now(),
    addedAt: String(item.added_at) || String(item.created_at)
      ? new Date(String(item.added_at) || String(item.created_at)).getTime()
      : Date.now(),
