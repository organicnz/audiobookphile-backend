...
import { MobileBookModel as MobileBook, MobileLibraryModel as MobileLibrary } from '../../../src/types/schemas.ts'
...
function _formatCoverPath(rawPath: string | null | undefined, itemId: string): string | null {
  ...
  return {
    ...
    settings: {
      coverAspectRatio: Number(
        (lib.settings as Record<string, unknown>)?.coverAspectRatio ?? (lib.settings as Record<string, unknown>)?.cover_aspect_ratio ?? 1
      ),
      ...
      skipMatchingMediaWithAsin: Boolean(
        ...
        (lib.settings as Record<string, unknown>)?.skip_matching_media_with_asin ??
        false
      ),
      skipMatchingMediaWithIsbn: Boolean(
        (lib.settings as Record<string, unknown>)?.skipMatchingMediaWithIsbn ??
        (lib.settings as Record<string, unknown>)?.skip_matching_media_with_isbn ??
        false
      ),
      autoScanCronExpression:
        String(
          (lib.settings as Record<string, unknown>)?.autoScanCronExpression ?? (lib.settings as Record<string, unknown>)?.auto_scan_cron_expression ?? ''
        ) || null
    },
    folders:
      lib.library_folders?.map((f: LibraryFolderRow) => ({
        id: f.id,
        fullPath: f.path || '',
...
export function mapBookForMobile(item: LibraryItemWithBooks, progressRecord?: MediaProgressRow | null): MobileBook {
  ...
  const authors = authorsList.map((ba) => ba.authors).filter(Boolean) as AuthorRow[]
  const authorNames = authors.map((a) => String(a.name))
  const rawAuthorFallback = String((bookRecord as any).author_names_first_last || '').trim()
  ...
  const authorNameLF = authors.map((a) => String(a.last_first || a.name)).join(', ') || authorName
  ...
  const totalBookDuration = Number(bookRecord.duration || Number(item.duration)) || 0
  ...
  const audioFiles =
    mappedFiles
      .map(({ af, meta, size }) => {
        ...
        return {
          id: String(af.id || af.ino || `${item.id}-track-${Number(af.track_index !== undefined ? af.track_index : af.index !== undefined ? af.index : 0)}`),
          ino: String(af.id || af.ino || `${item.id}-track-${Number(af.track_index !== undefined ? af.track_index : af.index !== undefined ? af.index : 0)}`),
  ...
  const userMediaProgress = progressRecord
    ? {
        ...
        duration: Number(progressRecord.duration) || Number(bookRecord.duration || Number(item.duration)) || 0,
        ...
        lastUpdate: new Date(progressRecord.last_update || Date.now()).getTime(),
        startedAt: progressRecord.started_at ? new Date(progressRecord.started_at).getTime() : null,
        finishedAt:
          progressRecord.is_finished && (progressRecord.finished_at || progressRecord.last_update)
            ? new Date(progressRecord.finished_at || progressRecord.last_update!).getTime()
            : null
      }
    : null
  ...
  return {
    ...
    mtimeMs: String(item.updated_at) || String(item.mtime) ? new Date(String(item.updated_at) || String(item.mtime)).getTime() : Date.now(),
    ctimeMs: String(item.created_at) || String(item.ctime) ? new Date(String(item.created_at) || String(item.ctime)).getTime() : Date.now(),
    birthtimeMs: String(item.created_at) || String(item.birthtime) ? new Date(String(item.created_at) || String(item.birthtime)).getTime() : Date.now(),
    addedAt: String(item.added_at) || String(item.created_at) ? new Date(String(item.added_at) || String(item.created_at)).getTime() : Date.now(),
    updatedAt: String(item.updated_at) ? new Date(String(item.updated_at)).getTime() : Date.now(),
    ...
    media: {
      ...
      chapters: chapters,
      duration: Number(bookRecord.duration || Number(item.duration)) || totalCalculatedDuration,
      size: Number(item.size) || 0,
      coverPath: _formatCoverPath(item.cover_path || String(bookRecord.cover_path || '') || null, item.id),
