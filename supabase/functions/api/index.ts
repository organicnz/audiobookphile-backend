...
app.use('*', async (c, next) => {
  ...
  try {
    ...
    const isForgotPassword = urlObj.pathname.includes('/api/auth/forgot-password');
    const isResetPassword = urlObj.pathname.includes('/api/auth/reset-password');
    const isRefresh = urlObj.pathname.includes('/api/auth/refresh');
    
    if (!isGetCover && !isLogin && !isSignup && !isForgotPassword && !isResetPassword && !isRefresh) {
