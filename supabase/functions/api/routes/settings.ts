...
settingsRouter.get('/filesystem', (c) => {
  return c.json({ directories: [{ path: '/', dirname: '/', level: 0 }], posix: true })
})
...
settingsRouter.post('/api-keys', (c) => c.json({ apiKey: { id: 'mock', key: 'mock', name: 'Mock Key' } }))
settingsRouter.patch('/api-keys/:id', (c) => c.json({ apiKey: { id: 'mock', key: 'mock', name: 'Mock Key' } }))
...
// --- SHARE MEDIA ITEM ---
settingsRouter.post('/share/mediaitem', (c) => c.json({ id: 'mock-share', libraryItemId: 'mock', slug: 'mock-share', expiresAt: null, createdAt: Date.now() }))
settingsRouter.delete('/share/mediaitem/:id', (c) => c.json({ success: true }))

// --- FEEDS ---
settingsRouter.get('/feeds', (c) => c.json({ feeds: [] }))
settingsRouter.post('/feeds/:type/:id/open', (c) => c.json({ feed: { id: 'mock', entityId: 'mock', entityType: 'item', coverPath: '', episodes: [] } }))
settingsRouter.post('/feeds/:id/close', (c) => c.json({ success: true }))

// --- CUSTOM METADATA PROVIDERS ---
settingsRouter.get('/custom-metadata-providers', (c) => c.json({ customMetadataProviders: [] }))
settingsRouter.post('/custom-metadata-providers', (c) => c.json({ provider: { id: 'mock', name: 'Mock Provider' } }))
settingsRouter.delete('/custom-metadata-providers/:id', (c) => c.json({ success: true }))
