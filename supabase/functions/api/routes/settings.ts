...
settingsRouter.delete('/genres/:genre', async (c) => {
  ...
  const { data: books } = await supabase.from('library_items').select('id, genres')
  ...
  for (const book of books ?? []) {
    ...
    if (Array.isArray(genres) && genres.includes(genre)) {
      ...
      await supabase.from('library_items').update({ genres: newGenres }).eq('id', book.id)
      numItemsUpdated++
    }
  }
...
settingsRouter.delete('/tags/:tag', async (c) => {
  ...
  const { data: books } = await supabase.from('library_items').select('id, tags')
  let numItemsUpdated = 0
  for (const book of books ?? []) {
    const tags = book.tags as string[] | null
    if (Array.isArray(tags) && tags.includes(tag)) {
      const newTags = tags.filter((t) => t !== tag)
      await supabase.from('library_items').update({ tags: newTags }).eq('id', book.id)
      numItemsUpdated++
    }
  }
  return c.json({ numItemsUpdated })
