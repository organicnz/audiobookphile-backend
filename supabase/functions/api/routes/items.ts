...
itemsRouter.get('/check-existing', async (c) => {
  ...
  try {
    let query = supabase.from('library_items').select('media_id').eq('library_id', libraryId).eq('media_type', mediaType).eq('title', title)
    ...
    const { data: allBooks } = await supabase.from('library_items').select('media_id, title').eq('library_id', libraryId).eq('media_type', mediaType)
...
itemsRouter.get('/:id', async (c) => {
  ...
  const { data: item, error } = await supabase.from('library_items').select('*, book_authors(authors(*)), book_series(series(*))').eq('id', itemId).single()
...
itemsRouter.get('/:id/cover', async (c) => {
  ...
  if (!coverPath || coverPath.startsWith('/') || (coverPath === 'missing' && force)) {
    ...
    const firstAuthorName = ((Array.isArray(authorsObj) ? authorsObj[0]?.name : authorsObj?.name) as string) || ''
    ...
    if (title) {
      try {
        ...
        if (fetchRes?.cover?.buffer) {
          ...
          const { error: uploadError } = await adminClient.storage.from('covers').upload(coverPath, fileData, { upsert: true, contentType })
          if (uploadError) {
            console.error(`[items] Cover upload failed for ${itemId}:`, uploadError)
        ...
        if (e instanceof Error && e.message === 'RateLimitExceeded') {
          ...
          return c.json({ error: 'Rate limit exceeded while fetching cover' }, 429)
        } else {
          coverPath = 'missing'
          await adminClient
            .from('library_items')
...
const handleCoverUpload = async (c: Context) => {
  ...
  const { error: uploadError } = await adminClient.storage.from('covers').upload(storagePath, fileData, { upsert: true, contentType })
...
itemsRouter.delete('/:id/audio-files/:ino', async (c) => {
  ...
  const adminClient = createClient(supabaseUrl, serviceRoleKey)
  const { data: item } = await adminClient.from('library_items').select('audio_files').eq('id', itemId).single()
  ...
  if (fileToDelete?.metadata?.path) {
    await adminClient.storage.from('audio-files').remove([fileToDelete.metadata.path])
  }

  const updatedFiles = audioFiles.filter((f: any) => f.ino !== fileIno)
  await adminClient.from('library_items').update({ audio_files: updatedFiles }).eq('id', itemId)
...
itemsRouter.post('/batch', async (c) => {
  ...
  const itemIds: string[] = Array.isArray(body.itemIds) ? body.itemIds.slice(0, 50) : []

  if (itemIds.length === 0) {
    return c.json({ items: [] })
  }

  const { data: items, error } = await supabase.from('library_items').select('*, book_authors(authors(*)), book_series(series(*))').in('id', itemIds)
  ...
  const mappedItems = items.map((item) => mapBookForMobile(item as any, progressMap.get(item.id)))
