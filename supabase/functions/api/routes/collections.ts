...
collectionsRouter.post('/:id/items', async (c) => {
  ...
  const { data, error } = await supabase.from('collections').select('*, collection_items(*)').eq('id', collectionId).single()
  if (error) throw error
  return c.json(data)
})
...
collectionsRouter.delete('/:id/items/:itemId', async (c) => {
  ...
  await supabase.from('collection_items').delete().eq('collection_id', collectionId).eq('library_item_id', libraryItemId)
