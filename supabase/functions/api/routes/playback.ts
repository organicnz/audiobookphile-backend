...
playbackRouter.post('/items/:id/play', async (c) => {
  ...
  const session = await PlaybackService.startSession(
    supabase, user.id, itemId, null, deviceInfo, supportedMimeTypes, forceDirectPlay, forceTranscode
  )
...
playbackRouter.post('/items/:id/play/:episodeId', async (c) => {
  ...
  const session = await PlaybackService.startSession(
    supabase, user.id, itemId, episodeId, deviceInfo, supportedMimeTypes, forceDirectPlay, forceTranscode
  )
...
playbackRouter.post('/session/:id/sync', async (c) => {
  ...
  const result = await PlaybackService.syncSession(supabase, user.id, sessionId, currentTime, timeListened)
