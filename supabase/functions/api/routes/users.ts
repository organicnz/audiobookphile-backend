...
usersRouter.get('/', async (c) => {
  ...
  const formattedUsers = users.map((u: any) => ({
    ...
    token: '',
    permissions: {
      download: true, update: u.user_type === 'admin', delete: u.user_type === 'admin',
      upload: u.user_type === 'admin', accessAllLibraries: true, accessAllTags: true, accessExplicitContent: true
    },
...
usersRouter.post('/', async (c) => {
  ...
  const emailToUse = body.username.includes('@') ? body.username : `${body.username}@local.abs`
...
usersRouter.patch('/me/preferences', async (c) => {
  ...
  const newMetadata = { ...authUser.user.user_metadata, preferences: newPreferences }
  
  const { error } = await adminSupabase.auth.admin.updateUserById(user.id, { user_metadata: newMetadata })
  if (error) throw error
