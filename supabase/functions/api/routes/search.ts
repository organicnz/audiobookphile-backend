...
searchRouter.get('/history', async (c) => {
  ...
  const { data: history, error } = await supabase.from('search_history').select('*').eq('user_id', user.id).order('created_at', { ascending: false }).limit(20)
...
searchRouter.post('/history', async (c) => {
  ...
  await supabase.from('search_history').delete().eq('user_id', user.id).eq('query', body.query)
...
searchRouter.post('/smart', async (c) => {
      ...
      const aiRes = await fetch('https://open.bigmodel.cn/api/paas/v4/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `Extract key search terms, author name, and genre from this natural search query: "${queryText}". Return ONLY a JSON object: {"terms": ["..."], "author": "...", "genre": "..."}`
            }
          ],
          temperature: 0.1
        })
  ...
  let dbQuery = supabase.from('library_items').select('*, book_authors(authors(*)), book_series(series(*))')
  ...
  const formattedResults = (results || []).map((item) => mapBookForMobile(item as unknown as LibraryItemWithBooks))
