...
authorsRouter.get('/:id/image', async (c) => {
  ...
  const { data: author } = await supabase.from('authors').select('image_path').eq('id', authorId).single()

  if (!author || !author.image_path || author.image_path === 'missing') {
    return c.redirect('https://raw.githubusercontent.com/lucide-icons/lucide/main/icons/user.svg')
  }

  const { data } = supabase.storage.from('covers').getPublicUrl(author.image_path)
