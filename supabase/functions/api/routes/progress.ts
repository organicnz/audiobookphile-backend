...
progressRouter.patch('/me/progress/:id', async (c) => {
  ...
  const data = await upsertMediaProgress(supabase, user.id, libraryItemId, body.episodeId ?? null, {
...
progressRouter.patch('/me/progress-batch', async (c) => {
  ...
  for (const item of items) {
    await upsertMediaProgress(supabase, user.id, item.libraryItemId, item.episodeId ?? null, {
