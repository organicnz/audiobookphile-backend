...
import { LibraryItemWithBooks, mapBookForMobile, mapLibraryForMobile } from '../../api/mappers.ts'
...
librariesRouter.get('/', async (c) => {
  ...
  const formatted = libraries.map((l) => mapLibraryForMobile(l as unknown as LibraryWithFolders))
...
librariesRouter.post('/', async (c) => {
  ...
  return c.json(mapLibraryForMobile((fullLibrary || {}) as unknown as LibraryWithFolders))
})
...
librariesRouter.get('/:id/items', async (c) => {
  ...
  const isDesc = queryParams.get('desc') === '1' || queryParams.get('desc') === 'true'
  ...
  try {
    ...
    if (isFetchAll) {
      ...
      while (true) {
        ...
        batchQuery = batchQuery.order(dbSortField, { ascending: !isDesc }).range(currentOffset, currentOffset + CHUNK_SIZE - 1)
        ...
        items.push(...data)
        if (data.length < CHUNK_SIZE || (count !== null && items.length >= count)) {
          break
        }
      ...
      query = query.order(dbSortField, { ascending: !isDesc }).range(offset, offset + limit - 1)
    ...
    if (itemIds.length > 0) {
      const { data: progressData } = await supabase
        ...
        .is('episode_id', null)
      progressMap = new Map((progressData || []).map((p) => [p.library_item_id, p]))
    }

    const mappedItems = items.map((i) => mapBookForMobile(i as unknown as LibraryItemWithBooks, progressMap.get(i.id)))
...
librariesRouter.post('/:id/smart-sort', async (c) => {
  ...
  const { data: items, error } = await supabase.from('library_items').select('id, title, author_names_first_last, published_year').eq('library_id', libraryId)
...
librariesRouter.get('/:id/search', async (c) => {
  ...
  let itemsQuery = supabase.from('library_items').select('*, book_authors(authors(*)), book_series(series(*))').eq('library_id', libraryId)
  ...
  const formattedItems = (items || []).map((item) => {
    const mapped = mapBookForMobile(item as unknown as LibraryItemWithBooks, progressMap.get(item.id))

    let matchKey = 'title'
    let matchText = item.title || ''

    const narratorsStr = Array.isArray(item.narrators) ? item.narrators.join(', ') : typeof item.narrators === 'string' ? item.narrators : ''

    if (item.title && item.title.toLowerCase().includes(qLower)) {
      matchKey = 'title'
      matchText = item.title
    } else if (item.author_names_first_last && item.author_names_first_last.toLowerCase().includes(qLower)) {
      matchKey = 'authorName'
    ...
    } else if (item.description && item.description.toLowerCase().includes(qLower)) {
  ...
  const bookResults = formattedItems.filter((i) => i.libraryItem.mediaType !== 'podcast').slice(0, limit)
  const podcastResults = formattedItems.filter((i) => i.libraryItem.mediaType === 'podcast').slice(0, limit)
  ...
  const { data: seriesData } = await supabase
    .from('series')
    .select('*, book_series(sequence, library_items(id, title, cover_path, duration, updated_at, created_at))')
    .eq('library_id', libraryId)
    .ilike('name', `%${searchPattern}%`)
    .limit(limit)
  ...
  const seriesResults = (seriesData || []).map((s) => {
    const books = (s.book_series || []).map((bs: any) => {
      const book = Array.isArray(bs.library_items) ? bs.library_items[0] : bs.library_items
      return {
        id: book?.id || bs.library_item_id,
        sequence: bs.sequence,
        title: book?.title || '',
        addedAt: book?.created_at ? new Date(book.created_at).getTime() : undefined,
        updatedAt: book?.updated_at ? new Date(book.updated_at).getTime() : undefined,
        media: {
          id: book?.id,
  ...
  const genreResults = Array.from(genresMap.entries()).map(([name, numItems]) => ({ name, numItems }))
  const narratorResults = Array.from(narratorsMap.entries()).map(([name, numBooks]) => ({ name, numBooks }))
      ...
      while (true) {
        ...
        for (const item of items) {
          ...
          if (result && result.metadata) {
            const updates: Database['public']['Tables']['library_items']['Update'] = {}
            ...
            if (Object.keys(updates).length > 0) {
              await supabase.from('library_items').update(updates).eq('id', bookItem.id)
            }
...
librariesRouter.get('/:id/series', async (c) => {
  ...
  const {
    ...
    .from('series')
    .select('*, book_series(library_item_id, sequence, library_items(id, title, cover_path, duration, updated_at, created_at))', {
      count: 'exact'
    })
    .eq('library_id', libraryId)
  ...
  const results = (seriesRows || []).map((s) => {
    const books = (s.book_series || []).map((bs) => {
      ...
      const toMs = (v: unknown): number | undefined => (typeof v === 'string' && v ? new Date(v).getTime() || undefined : undefined)
      return {
        id: itemId,
        sequence: bs.sequence,
        title: book?.title || '',
...
librariesRouter.get('/:id/collections', async (c) => {
  ...
  const {
    ...
    .from('collections')
    .select('*, collection_items(library_item_id, order, library_items(id, title, cover_path, duration, updated_at, created_at))', { count: 'exact' })
    .eq('library_id', libraryId)
    .order(dbSortField, { ascending: !desc })
    .range(offset, offset + limit - 1)
  ...
  const results = (collectionRows || []).map((cObj) => {
    const books = (cObj.collection_items || []).map((cb) => {
      const book = Array.isArray(cb.library_items) ? cb.library_items[0] : cb.library_items
      const libraryItem = book
    ...
    // Sort books by order
    books.sort((a: { order?: number | null }, b: { order?: number | null }) => (a.order || 0) - (b.order || 0))
...
librariesRouter.get('/:id/playlists', async (c) => {
  ...
  const {
    ...
    .from('playlists')
    .select('*, playlist_media_items(media_item_id, order, library_items(id, title, cover_path, duration, updated_at, created_at))', { count: 'exact' })
    .eq('library_id', libraryId)
    .order(dbSortField, { ascending: !desc })
    .range(offset, offset + limit - 1)
  ...
  const results = (playlistRows || []).map((pObj) => {
    const items = (pObj.playlist_media_items || []).map((pm) => {
      const book = Array.isArray(pm.library_items) ? pm.library_items[0] : pm.library_items
      const libraryItem = book
    ...
    // Sort items by order
    items.sort((a: { order?: number | null }, b: { order?: number | null }) => (a.order || 0) - (b.order || 0))
...
librariesRouter.get('/:id/personalized', async (c) => {
  ...
  const { data: libraryItemRows } = await supabase.from('library_items').select('id').eq('library_id', libraryId)
  ...
  let recentProgressMap = new Map<string, Database['public']['Tables']['media_progress']['Row']>()
  if (recentItemIds.length > 0) {
    const { data: recentProgressData } = await supabase
      ...
      .is('episode_id', null)
    recentProgressMap = new Map((recentProgressData || []).map((p) => [p.library_item_id, p]))
  }

  const formattedRecent = (recentItems || []).map((item) => mapBookForMobile(item as unknown as LibraryItemWithBooks, recentProgressMap.get(item.id) ?? null))
  ...
  const [continueResult, listenAgainResult] =
    libraryItemIds.length > 0
      ? await Promise.all([
          supabase
            .from('media_progress')
            .select('*, library_items(*, book_authors(authors(*)), book_series(series(*)))')
            .eq('user_id', user.id)
  ...
  type ProgressWithItem = Database['public']['Tables']['media_progress']['Row'] & { library_items?: any }

  const continueItems = ((continueResult.data || []) as unknown as ProgressWithItem[])
    .filter((p) => p.library_items)
    .map((p) => mapBookForMobile(p.library_items as unknown as LibraryItemWithBooks, p))
    .filter(Boolean)

  const listenAgainItems = ((listenAgainResult.data || []) as unknown as ProgressWithItem[])
    .filter((p) => p.library_items)
    .map((p) => mapBookForMobile(p.library_items as unknown as LibraryItemWithBooks, p))
    .filter(Boolean)
...
librariesRouter.post('/:id/deduplicate', async (c) => {
  ...
  try {
    const { data: removedCount, error } = await (supabase as any).rpc('deduplicate_library_items')
    if (error) throw error
