...
authRouter.post('/login', async (c) => {
  ...
  if (!username.includes('@')) {
    const { data: profile } = await adminSupabase.from('profiles').select('id').eq('username', username).single()
    if (profile?.id) {
  ...
  const { data: authData, error: authError } = await supabase.auth.signInWithPassword({ email: emailToUse, password })
  if (authError || !authData.user) return c.json({ error: { message: 'Invalid credentials' } }, 401)

  const { data: profile } = await adminSupabase.from('profiles').select('*').eq('id', authData.user.id).single()
  ...
  const userPayload = {
    user: {
      ...
      isActive: true, isLocked: false, lastSeen: Date.now(),
      createdAt: new Date(profile?.created_at || authData.user.created_at).getTime(),
      permissions: {
        download: true, update: profile?.user_type === 'admin', delete: profile?.user_type === 'admin',
        upload: profile?.user_type === 'admin', accessAllLibraries: true, accessAllTags: true, accessExplicitContent: true
      },
      librariesAccessible: [], itemTagsAccessible: []
    },
    userDefaultLibraryId: profile?.default_library_id || null,
    serverSettings: {}, source: 'local'
  }
...
authRouter.post('/signup', async (c) => {
  ...
  if (username && authData.user) {
    const adminSupabase = createClient(supabaseUrl, serviceRoleKey)
    await adminSupabase.from('profiles').update({ username }).eq('id', authData.user.id)
  }
...
authRouter.post('/auth/forgot-password', async (c) => {
  ...
  const body = await c.req.json()
  const { email } = body
  const siteUrl = c.req.header('origin') || Deno.env.get('NEXT_PUBLIC_SITE_URL') || 'http://localhost:3000'
...
authRouter.post('/auth/refresh', async (c) => {
  ...
  const supabaseUrl = c.get('supabaseUrl')
  const serviceRoleKey = c.get('serviceRoleKey')
  const refreshToken = c.req.header('x-refresh-token') || (await c.req.json().catch(() => ({}))).refreshToken
  if (!refreshToken) throw new Error('Missing refresh token')
  
  const { data, error } = await supabase.auth.refreshSession({ refresh_token: refreshToken })
  if (error || !data.session) return c.json({ error: { message: 'Invalid refresh token' } }, 401)
  ...
  const userPayload = {
    user: {
      ...
      mediaProgress: [], seriesHideFromContinueListening: [], bookmarks: [],
      isActive: true, isLocked: false, lastSeen: Date.now(),
      createdAt: new Date(profile?.created_at || data.user!.created_at).getTime(),
      permissions: {
        download: true, update: profile?.user_type === 'admin', delete: profile?.user_type === 'admin',
        upload: profile?.user_type === 'admin', accessAllLibraries: true, accessAllTags: true, accessExplicitContent: true
      },
...
authRouter.post('/authorize', async (c) => {
  ...
  if (userError || !user) {
    console.error(`[auth.ts] /authorize getUser failed:`, userError, `user is null?`, !user)
