...
downloadsRouter.get('/:id/download', async (c) => {
  ...
  if (itemError || !item) {
    return c.json({ error: `Library item not found: ${itemError?.message || ''}` }, 404)
  }

  let audioFilesList = ((item as Record<string, unknown>)?.audio_files || []) as Record<string, unknown>[]

  if (!audioFilesList.length && Array.isArray((item as Record<string, unknown>)?.library_files)) {
    const libraryFiles = (item as Record<string, unknown>).library_files as Record<string, unknown>[]
    const audioExtensions = ['.mp3', '.m4b', '.m4a', '.aac', '.flac', '.ogg', '.opus', '.wma']
    audioFilesList = libraryFiles
      .filter((lf) => {
        ...
        return audioExtensions.some((e) => ext.endsWith(e) || relPath.endsWith(e))
      })
      .map((lf, idx) => {
        const metadata = (lf.metadata as Record<string, unknown>) || {}
        return {
  ...
  for (let i = 0; i < sortedAudioFiles.length; i++) {
    ...
    try {
      ...
      console.warn(`[DownloadsRoute] Missing storage file at "${storagePath}": ${signErr.message}. Skipping track.`)
  ...
  if (tracks.length === 0) {
    return c.json({ error: 'All audio files are missing from storage. Cannot download.' }, 404)
  }
...
downloadsRouter.get('/:id/file/:fileId/download', async (c) => {
  ...
  const { data: item, error: itemError } = await supabase.from('library_items').select('audio_files').eq('id', libraryItemId).maybeSingle()
  ...
  const file = audioFiles.find((f: any) => String(f.ino) === fileId || String(f.id) === fileId)

  if (!file) {
    return c.json({ error: 'File not found' }, 404)
  }

  const storagePath = String(file.metadata?.path || file.storage_path || file.path || '')
  if (!storagePath) {
  ...
  try {
    const signedUrl = await storage.getSignedUrl(storagePath, DOWNLOAD_EXPIRY_SECONDS)
