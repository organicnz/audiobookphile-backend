...
metadataRouter.patch('/narrators/:id', async (c) => {
  return c.json({ error: 'Not implemented (narrators table does not exist)' }, 501)
})
...
metadataRouter.delete('/genres/:id', async (c) => {
  return c.json({ error: 'Not implemented (genres table does not exist)' }, 501)
})
...
metadataRouter.post('/match-book', async (c) => {
  ...
  try {
    ...
    const olRes = await fetch(`https://openlibrary.org/search.json?${query.toString()}`, { signal: AbortSignal.timeout(8000) })
          ...
          const aiRes = await fetch('https://open.bigmodel.cn/api/paas/v4/chat/completions', {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: 'user',
                  content: `Provide accurate metadata for the audiobook "${title}" by "${
                    author || 'Unknown'
