...
Every mutating/server-global endpoint must have an explicit owner: **self**
(RLS-protected, user-scoped), **admin** (`requireAdminRole` — admin or root,
DB-fresh per request), or **public** (documented). Auth middleware runs on all
routes except `publicAuthRoutes`.
...
| items.ts        | DELETE `/ :id` (admin; atomic DB delete; `hardDelete=1` creates durable B2/cover cleanup state that is idempotently retryable with the same DELETE), DELETE `/ :id/cover`, POST/PATCH `/ :id/cover`, sync-covers, sync-durations, sync-insights                                                            |
...
aiService.ts `POST /api/ai/insights`, items.ts `handleChapterAI`
(`/api/chapter-ai`, `/api/items/:id/chapters/ai`), search.ts `/smart`,
`/semantic`, `/search-semantic`, `/generate-embedding`. Cache-gated
(book_insights) to bound cost. Revisit if quota abuse.

## Public (auth-skipped)

`/api/health` (includes `contracts`: zod shape checks per endpoint,
`ok | missing | wrong-shape` + timings; nested self-check guarded by
`x-contract-check` header), login/refresh/authorize/logout/
forgot/reset/change-password/magic-link/verify/verify-token/signup, cover &
author images (GET).

## Alias routes

`/api/*` paths are aliases of the canonical runtime-stripped paths (Edge Runtime
strips the `/functions/v1/api` prefix). Alias hits are logged once per path
(`route-alias`, includes the canonical path) to observe migration off the
prefix.

## Notes / debt

- items.ts DELETE `/ :id/audio-files/:ino` uses an inline admin|root DB check —
  migrate to `requireAdminRole` next pass.
- `GET /api/users` remains the only admin-gated read on the user list — keep
  behind admin.
