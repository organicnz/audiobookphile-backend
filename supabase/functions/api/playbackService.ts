...
export class PlaybackService {
  static async startSession(
    ...
    let audioFilesList = ((item as Record<string, unknown>)?.audio_files || []) as Record<string, unknown>[]

    // Fallback: If audio_files is empty, extract audio files from library_files
    if (!audioFilesList.length) {
      const libraryFiles = ((item as Record<string, unknown>)?.library_files || []) as Record<string, unknown>[]
      ...
      const extracted = libraryFiles
        ...
        .map((lf, idx) => {
          ...
          return {
            ...
            filename: String(metadata.filename || metadata.relPath || `Track ${idx + 1}`)
          }
        })
    ...
    const sortedAudioFiles = [...audioFilesList]
      .map((af) => {
        ...
        return {
          ...af,
          index: af.track_index !== undefined ? Number(af.track_index) : af.index !== undefined ? Number(af.index) : 0,
    ...
    for (let i = 0; i < sortedAudioFiles.length; i++) {
      ...
      const metadata = ((af as any).metadata as Record<string, unknown>) || {}
      const storagePath = String(metadata.path ?? (af as any).storage_path ?? (af as any).path ?? '')
      ...
      if (!isMissing && finalSignedUrl) {
        audioTracks.push({
          ...
          duration: duration,
          title: String(metadata.filename || (af as any).filename || `Track ${i + 1}`),
    ...
    let progressQuery = supabase.from('media_progress').select('*').eq('user_id', userId).eq('library_item_id', libraryItemId)
    ...
    const authors = bookAuthors.map((ba) => ba.authors as Record<string, unknown>).filter(Boolean)
    ...
    const chapters = chaptersList
      .map((ch, index) => ({
        id: ch.chapter_index !== undefined ? Number(ch.chapter_index) : typeof ch.id === 'number' ? ch.id : index,
  ...
  static async syncSession(
    ...
    try {
      await upsertMediaProgress(supabase, userId, libraryItemId, episodeId || null, {
        currentTime,
        duration,
        progress
      })
  ...
  static async bulkSyncSessions(
    ...
    if (progressItems.length > 0) {
      try {
        ...
        console.warn(`[PlaybackService] Bulk progress upsert failed, falling back to individual:`, e)
        // Fall back to individual upsert
        for (const item of progressItems) {
          try {
            await upsertMediaProgress(supabase, userId, item.libraryItemId, item.episodeId, {
            ...
            console.error(`[PlaybackService] Individual progress upsert failed for ${item.sessionId}:`, individualErr)
          }
        }
      }
    }
    ...
    const sessionUpdates = new Map<string, { currentTime: number; timeListened: number; originalSessionId: string }>()
    ...
    if (sessionUuids.length > 0) {
      try {
        const { data: existingSessions } = await supabase.from('playback_sessions').select('id, time_listening').in('id', sessionUuids)

        const existingMap = new Map((existingSessions || []).map((s) => [s.id, s.time_listening || 0]))

        // Update concurrently
        const updatePromises = Array.from(sessionUpdates.entries()).map(async ([sessionUuid, update]) => {
          ...
          try {
            ...
            console.error(`[PlaybackService] Failed to update playback_session ${sessionUuid}:`, sessionErr)
        ...
        console.error(`[PlaybackService] Failed to fetch or bulk update playback_sessions:`, e)
        return {
          success: false,
