...
export const authErrorHandlers = {
  ...
  SESSION_EXPIRED: () => new ApiError('Session expired', 'SESSION_EXPIRED', 401),
  NO_SESSION: () => new ApiError('No session', 'NO_SESSION', 401),
  USER_NOT_FOUND: () => new ApiError('User not found', 'USER_NOT_FOUND', 401),
  USER_DEACTIVATED: () => new ApiError('Account deactivated', 'USER_DEACTIVATED', 401),
}
 ...
 */
export async function checkUserStatus(adminSupabase: any, userId: string): Promise<boolean> {
