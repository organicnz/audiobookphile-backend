...
serve(async (req) => {
  ...
  try {
    ...
    if (!query) {
      return new Response(JSON.stringify({ error: 'Search query is required in body or query string' }), { status: 400, headers: corsHeaders })
    }
    ...
    const { data: matches, error: matchError } = await supabase.rpc('search_library_items_by_embedding', {
