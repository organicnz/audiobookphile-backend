...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: matches, error: matchError } = await supabase.rpc('match_library_items_hybrid', {
      query_text: query,
    ...
    const { data: items, error: itemsError } = await supabase.from('library_items').select('*, book_authors(authors(*)), book_series(series(*))').in('id', ids)
    ...
    const formattedResults = sortedItems.map((item) => mapBookForMobile(item as unknown as LibraryItemWithBooks))
