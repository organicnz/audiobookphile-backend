...
if ! echo "$COMMIT_MSG" | grep -qE "$CONVENTIONAL_REGEX"; then
    ...
    echo ""
    echo "Examples:"
    echo "  feat(api): add dedicated book insights edge function"
    echo "  fix(auth): enforce strict RLS and JWT verification"
    echo "  perf: optimize database query execution"
    echo "  chore: update Deno dependencies"
    echo "--------------------------------------------------------"
    exit 1
fi
