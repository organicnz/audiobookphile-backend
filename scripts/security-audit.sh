...
SQL_INJECTION=$(grep -r -n -E '\.query\([^)]*\+|from\([^)]*\+' supabase/functions/ 2>/dev/null || true)
...
CREATE_CLIENT_FILES=$(grep -r -l 'createClient' supabase/functions/ 2>/dev/null | grep '\.ts$' || true)
